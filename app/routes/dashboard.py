...
def sale_time(d,an):
    if authentication():
        try:
            ...
            data = []
            valor = []
            cur2 = db.session.query(Quotation.dateO).filter(and_(Quotation.delivery >= anio+str("-01-01"), Quotation.delivery <= anio+str("-12-31"))).group_by(Quotation.dateO).order_by(Quotation.dateO).all()
            cur = db.session.query(func.sum(Quotation.valueT).label('mayor')).filter(Quotation.request_id == Request.id, or_(Request.state == 'Aceptado', Request.state == 'Completado')).filter(and_(Quotation.delivery >= anio+str("-01-01"), Quotation.delivery <= anio+str("-12-31"))).group_by(Quotation.dateO).order_by(Quotation.dateO).all()
            for i in cur:
                data.append(i.mayor)
            for i in cur2:
...
def sale_timeline(d, an):
    if authentication():
        try:
            ...
            data4 = []
            valor = []
            ejex = db.session.query(Quotation.dateO).filter(Quotation.request_id == Request.id, or_(Request.state == 'Aceptado', Request.state == 'Completado')).filter(and_(Quotation.delivery >= anio+str("-01-01"), Quotation.delivery <= anio+str("-12-31"))).group_by(Quotation.dateO).order_by(Quotation.dateO).all()
            total = db.session.query(func.sum(Quotation.valueT).label("valorT")).filter(Quotation.request_id == Request.id, or_(Request.state == 'Aceptado', Request.state == 'Completado')).filter(and_(Quotation.delivery >= anio+str("-01-01"), Quotation.delivery <= anio+str("-12-31"))).group_by(Quotation.dateO).order_by(Quotation.dateO).all()
            prom = db.session.query(func.avg(Quotation.valueT).label("valorP")).filter(Quotation.request_id == Request.id, or_(Request.state == 'Aceptado', Request.state == 'Completado')).filter(and_(Quotation.delivery >= anio+str("-01-01"), Quotation.delivery <= anio+str("-12-31"))).group_by(Quotation.dateO).order_by(Quotation.dateO).all()
            maxi = db.session.query(func.max(Quotation.valueT).label("valorM")).filter(Quotation.request_id == Request.id, or_(Request.state == 'Aceptado', Request.state == 'Completado')).filter(and_(Quotation.delivery >= anio+str("-01-01"), Quotation.delivery <= anio+str("-12-31"))).group_by(Quotation.dateO).order_by(Quotation.dateO).all()
            mini = db.session.query(func.min(Quotation.valueT).label("valorMi")).filter(Quotation.request_id == Request.id, or_(Request.state == 'Aceptado', Request.state == 'Completado')).filter(and_(Quotation.delivery >= anio+str("-01-01"), Quotation.delivery <= anio+str("-12-31"))).group_by(Quotation.dateO).order_by(Quotation.dateO).all()
            for i in ejex:
                valor.append(i.dateO)
            for i in total:
